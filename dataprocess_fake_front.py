...
#     ff = np.load('/data/lchen63/grid/zip/video/s20/video/mpg_6000/bbad3n.npy')[0]
    ...
    srt_path = video_path[:-4] +  '_sRT_fake_front.npy'
    front_path =video_path[:-4] +  '_fake_front.npy'
    fake_lmark_path = os.path.join(Path(video_path).parent, 'predicted_lmarks.npy')     
    
    t_lmark = np.load(fake_lmark_path)
...
def get_animation(  obj_path= None, reference_prnet_lmark_path= None, img_path = None,  \
    ...
    if video_path != None:
        ...
        # rt_path  = video_path[:-4] + '_sRT_front.npy'  # if it is real front
        rt_path  = video_path[:-4] + '_sRT_fake_front.npy'  # if it is fake_front
        lmark_path  = video_path[:-4] +'_front.npy'
        ani_video_name = video_path[:-4] + '_fake_front.mp4'
        if os.path.exists( ani_video_name):
            print ('=====')
    ...
    if video_path != None: 
        command = 'ffmpeg -framerate 25 -i '  + temp_path + '/%5d.png  -c:v libx264 -y -vf format=yuv420p ' +  ani_video_name
    else:
        command = 'ffmpeg -framerate 25 -i '  + temp_path + '/%5d.png  -c:v libx264 -y -vf format=yuv420p ' +  ani_video_name
    os.system(command)
        ...
        for i_path in os.listdir(v_path):
            ...
            if i_path[-3:] == 'mpg' or i_path[-3:] == 'mp4' or i_path[-3:] == 'avi':
                ...
                # except:
                    ...
                    # continue
    print (gg_list)
    with open(os.path.join( '/home/cxu-serve/p1/lchen63/voxceleb/txt','vox_audio' + '_key_frame.pkl'), 'wb') as handle:
        pickle.dump(gg_list, handle, protocol=pickle.HIGHEST_PROTOCOL)

def get_animation_pickle():

    pkl_name = os.path.join( '/home/cxu-serve/p1/lchen63/voxceleb/txt','vox_audio' +  '_key_frame.pkl')
    ...
    for gg in gggdata:        
        ...
        reference_id = gg[1]
        try:
            get_animation(video_path = v_path,key_id = reference_id)
        except:
            print ('++++++')
            continue
...
convert_folder('/home/cxu-serve/p1/lchen63/voxceleb/unzip/test_prediction_video2/')
